...
describe Stellar::SEP10 do
  ...
  describe '#read_challenge_tx' do
    ...
    it 'throws an error if transaction sequence number is different to zero' do
      ...
      end.to raise_error(Stellar::InvalidSep10ChallengeError, /The transaction sequence number should be zero/)
    end
    ...
    it 'throws an error if transaction source account is different to server account id' do
      ...
      end.to raise_error(Stellar::InvalidSep10ChallengeError, /The transaction source account is not equal to the server's account/)
    end
    ...
    it "throws an error if transaction doesn't contain any operation" do
      ...
      expect do
        sep10.read_challenge_tx(challenge_xdr: envelope.to_xdr(:base64), server: server)
      end.to raise_error(Stellar::InvalidSep10ChallengeError, /The transaction should contain only one operation/)
    end
    ...
    it 'throws an error if operation does not contain the source account' do
      ...
      expect do
        sep10.read_challenge_tx(challenge_xdr: envelope.to_xdr(:base64), server: server)
      end.to raise_error(Stellar::InvalidSep10ChallengeError, /The transaction's operation should contain a source account/)
    end
    ...
    it 'throws an error if operation is not manage data' do
      ...
      expect do
        sep10.read_challenge_tx(challenge_xdr: envelope.to_xdr(:base64), server: server)
      end.to raise_error(Stellar::InvalidSep10ChallengeError, /The transaction's operation should be manageData/)
    end
  ...
  describe '#verify_challenge_tx_threshold' do
    it 'raises transaction not signed by server' do
      ...
      challenge_envelope.signatures = [client_kp_a, client_kp_b, client_kp_c].map do |kp|
    ...
    it "raises error when signers don't meet threshold" do
      ...
      expect do
        ...
        Stellar::InvalidSep10ChallengeError,
        format('signers with weight %d do not meet threshold %d.', 1, 7)
      )
    end
  ...
  describe '#verify_challenge_tx' do
    ...
    it 'raises not signed by client' do
      ...
      expect do
        ...
        Stellar::InvalidSep10ChallengeError,
        format('Transaction not signed by client: %s', client_kp.address)
      )
    end
  end
